...
allowed-tools: ["Bash(cavekit config:*)"]
---
...
2. If `--global` is present, run `cavekit config set bp_model_preset {parsed preset name} --global`
3. Otherwise run `cavekit config set bp_model_preset {parsed preset name} --project`
